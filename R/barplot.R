...
barplot <- function(data, title="", xlab="", ylab="", barIndex=NULL, label="*") {
    ...
    p <- ggplot(d, aes(sample, mean, fill=sample, width=0.5))
    p <- p+geom_errorbar(aes(ymin=mean-sd, ymax=mean+sd, width=0.2),
                         position=position_dodge(width=0.8))
    p <- p+geom_bar(stat="identity", position=position_dodge(width=0.8)) ##, colour="black")
...
##' @importFrom ggplot2 aes
addLabel <- function(p, barIndex=c(1,2), label="*") {
    d <- p$data
    lw <- d$mean+d$sd
    lw <- lw[barIndex] * 1.02
    up <- max(lw * 1.1)
    p <- p+geom_segment(aes(x=barIndex, y=lw, xend=barIndex, yend=max(up)))
    p <- p+geom_segment(aes(x=barIndex[1], y=up, xend=barIndex[2], yend=up))
    pos <- up - (up - max(lw))/6
